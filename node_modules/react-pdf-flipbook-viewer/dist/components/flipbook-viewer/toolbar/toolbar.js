...
const Toolbar = ({ flipbookRef, containerRef, screenfull, pdfDetails, viewerStates, shareUrl, disableShare, setViewerStates }) => {
    ...
    const fullScreen = useCallback(() => {
        if (screenfull && screenfull.isEnabled && containerRef.current) {
            screenfull.toggle(containerRef.current, { navigationUI: "hide" }).catch((err) => {
    ...
    return (React.createElement("div", { className: "fixed bottom-0 left-0 right-0 w-full bg-background shadow-lg z-40 p-2 print:hidden" },
        ...
        React.createElement("div", { className: "flex items-center justify-between gap-x-1 sm:gap-x-2" },
            React.createElement("div", { className: "hidden md:flex items-center flex-shrink-0 min-w-[100px]" },
                " ",
                pdfDetails?.totalPages > 0 && (React.createElement("p", { className: 'text-xs font-medium text-muted-foreground whitespace-nowrap' },
                    "Page ",
                    pagesInFlipViewDisplay,
                    " of ",
                    pdfDetails?.totalPages))),
            React.createElement("div", { className: "md:hidden flex-shrink-0 min-w-[100px]" }),
            React.createElement("div", { className: "flex-grow flex items-center justify-center gap-1 sm:gap-2" },
                React.createElement(Button, { onClick: () => {
                        if (flipbookRef.current?.pageFlip) {
                            screenWidth < 768 ? flipbookRef.current.pageFlip().turnToPrevPage() : flipbookRef.current.pageFlip().flipPrev();
                        }
                    }, disabled: !canFlipPrev, variant: 'secondary', size: 'icon', className: 'size-8 min-w-8', "aria-label": "Previous page" },
                    React.createElement(ChevronLeft, { className: "size-4 min-w-4" })),
                React.createElement("div", { className: "md:hidden" }, pdfDetails?.totalPages > 0 && (React.createElement("p", { className: 'text-xs font-medium text-muted-foreground whitespace-nowrap' },
                    pagesInFlipViewDisplay,
                    "/",
                    pdfDetails?.totalPages))),
                React.createElement(Button, { onClick: () => {
                    ...
                    }, disabled: !canFlipNext, variant: 'secondary', size: 'icon', className: 'size-8 min-w-8', "aria-label": "Next page" },
                    React.createElement(ChevronRight, { className: "size-4 min-w-4" }))),
            React.createElement("div", { className: "flex items-center flex-shrink-0 justify-end gap-1 sm:gap-2 min-w-[100px]" },
                " ",
                React.createElement(Zoom, { zoomScale: viewerStates.zoomScale, screenWidth: screenWidth }),
                !disableShare && (React.createElement(Share, { shareUrl: shareUrl })),
