...
const Button = forwardRef(({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    if (asChild) {
        if (React.Children.count(children) !== 1 || !React.isValidElement(children)) {
            console.error("Button component with `asChild` prop expects a single ReactElement child.");
            throw new Error("Button component with `asChild` prop expects a single ReactElement child.");
        }
        ...
        const mergedProps = {
            ...
            className: cn(cvaGeneratedClasses, // Base styles from CVA
    ...
    return (React.createElement("button", { className: cn(cvaGeneratedClasses, className ?? ''), ref: ref, ...props },
        children,
        " "));
});
