...
import { ArrowRight, BookOpen, Feather, MessageSquare, Sparkles } from 'lucide-react';
        ...
        <div className="marquee__content">
          {titles.map(item => (
            <div key={`${item.id}-1`} className="flex flex-col items-center mx-6 space-y-2">
              <img src={item.picture} alt={item.title} className="w-36 h-32 object-cover rounded-xl shadow-lg" />
              <span className="text-base font-medium text-foreground text-center whitespace-nowrap">{item.title}</span>
            </div>
          ))}
        </div>
...
const HomePage = () => {
  return (
    <div className="space-y-24 md:space-y-32 overflow-hidden relative">
      ...
      {/* Hero Section */}
      <section id="home" className="text-center py-8 md:py-2 relative z-10">
        <motion.div
